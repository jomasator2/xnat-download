...
from .convert_xml2image import xml2image
...
class AssessorsResources(dict):
        ...
        if verbose:
            print(
                format_message(
                    self.level_verbose,
                    self.level_tab,
                    f"assessor resources: {self['label']}",
                ),
                end=" ----> ",
                flush=True,
            )
    ...
    def download_roi_files(
        ...
        roi = try_to_request(
            self["assessors"]["session"]["subject"]["project"].interface, url_roi
        )
